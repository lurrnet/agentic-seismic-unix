...
def mean_amplitude_spectrum(traces, dt_s, normalize=True):
    """Return the mean absolute amplitude spectrum across traces.

    normalize=True is useful for visualization and spectral-shape summaries.
    normalize=False preserves relative amplitude between datasets and must be
    used for quantitative before/after QC ratios.
    """
...
def summarize_frequency_content(traces, dt_s):
    freq, amp = mean_amplitude_spectrum(traces, dt_s, normalize=True)
